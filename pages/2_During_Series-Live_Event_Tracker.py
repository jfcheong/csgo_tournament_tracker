...
def load_state():
    with open('C:/Users/jfche/Downloads/grid/csgo/CCT-Online-Finals-1/2578928_state.json', 'r') as json_file:
        state = dict(json.load(json_file))
    return state
...
st.markdown(f"### Date of Match: {match_date}")
st.markdown(f"### Match format: {format}")
        ...
        with placeholder.container():
            ...
            with (col_t):
                ...
                for i in range(len(pha_filtered)):
                    ...
                    lo_mapped = lo_filtered[['loadout.primary', 'loadout.secondary', 'loadout.melee', 'loadout.helm', 'loadout.kevlarVest']].applymap(weapons_dict.get).rename(columns={'loadout.primary': 'Primary', 'loadout.secondary': 'Secondary', 'loadout.melee': 'Melee', 'loadout.helm': 'Helm', 'loadout.kevlarVest': 'Kevlar'})
                    st.markdown(f"##### Player {i+1}: :blue[{pha_filtered.loc[i, 'name']}] :red[({get_player_kda(kda, gti_latest_round, pha_filtered, i)})]")
                    st.dataframe(lo_mapped, column_config={
                        ...
                        "Helm": st.column_config.ImageColumn(label="Helm", width='small'),
                        "Kevlar": st.column_config.ImageColumn(label="Kevlar", width='small'),
...
with postround_tab:
    ...
    for event_num in range(len(end_events_list)):
        ...
        with placeholder2.container():
            ...
            bomb_info = ['Bomb_Planted', 'Bomb_StartDefuse_WithKit', 'Bomb_StartDefuse_WithoutKit', 'Bomb_Defused', 'Bomb_Exploded']

            team1_df = player_kda.loc[player_kda.team == team1].drop(["map_name", "team"], axis=1
                                                                     ).rename(columns={'map_seq': 'Map_Number', 'kills': 'Kills', 'killAssistsGiven': 'Assists', 'multikills': 'Multi_Kills',
                                                                                       'deaths': 'Deaths', 'adr': 'ADR', 'name': 'Player', 'objectives.plantBomb': 'Bomb_Planted',
                                                                                       'objectives.beginDefuseWithKit': 'Bomb_StartDefuse_WithKit', 'objectives.beginDefuseWithoutKit': 'Bomb_StartDefuse_WithoutKit',
