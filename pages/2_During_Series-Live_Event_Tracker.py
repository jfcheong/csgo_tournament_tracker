...
def clean_text(s):
    """ Cleans text into human readable format """
    s = s.replace("_", " ")
...
def format_items(loadout, exclude_col=["loadout.primary", "loadout.secondary"]):
    ...
    return [cleaned if (cleaned := [elem for elem in sublist 
                                                    if elem is not None])
...
with preround_tab:
    ...
    for event_num in range(len(r2_events_list)):
        ...
        # Format economy df
        economy["Equipment"] = format_items(lo)
        ...
        for event_num in range(len(r2_events_list)):
            ...
            with placeholder.container():
                ...
                with col1:
                    ...
                    kills_df = get_weapons_img_path(kills_df, ["weapon"])
                ...
                with (col_t):
                    ...
                    for i in range(len(pha_filtered)):
                        ...
                        lo_mapped = get_weapons_img_path(lo_filtered, ['loadout.primary', 'loadout.secondary', 'loadout.melee']).rename(columns={'loadout.primary': 'Primary', 'loadout.secondary': 'Secondary', 'loadout.melee': 'Melee'})
