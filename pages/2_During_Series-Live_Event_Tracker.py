...
def load_round2_events():
    with open('../2579089_events_round_2_sample.jsonl', 'r') as jsonl_file:
        round2_events = json.load(jsonl_file)
    return round2_events

@st.cache_data  # 👈 Add the caching decorator
def load_endofround_events():
    with open('../2579089_events_end_of_round_sample.jsonl', 'r') as jsonl_file:
...
state = load_state()
round2_events = load_round2_events()
endofround_events = load_endofround_events()
...
round_num = round2_events[0]["events"][-1]["seriesState"]["games"][-1]["segments"][-1]["id"].replace('round-', '')
...
with postround_tab:
    ...
    for event_num in range(len(endofround_events)):
        selected_event_list = endofround_events[event_num]["events"]
...
for event_num in range(len(round2_events)):
    # Load event data
    selected_event_list = round2_events[event_num]["events"]
    for j in range(len(selected_event_list)):
        selected_event = selected_event_list[j]
